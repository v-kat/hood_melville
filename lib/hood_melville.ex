...
defmodule IncrementalAppend do
  @type append_state ::
          {:reversing, [term()], [term()], [term()]}
          | {:appending, [term()], [term()]}
          | {:done, [term()]}

  @spec start_append([term()], [term()]) :: append_state()
  def start_append(xs, ys) do
    {:reversing, xs, [], ys}
  end

  @spec exec(append_state()) :: append_state()
  def exec({:reversing, [x | xs], ys, zs}) do
    {:reversing, xs, [x | ys], zs}
  end

  def exec({:reversing, [], ys, zs}) do
    {:appending, ys, zs}
  end

  def exec({:appending, [x | xs], ys}) do
    {:appending, xs, [x | ys]}
  end

  def exec({:appending, [], ys}) do
    {:done, ys}
  end

  def exec({:done, any}) do
    {:done, any}
  end
end

defmodule IncrementalReverse do
  @type reverse_state :: {:working, [term()], [term()]} | {:done, [term()]}

  @spec start([term()]) :: reverse_state()
  def start(xs) do
    {:working, xs, []}
  end

  @spec exec(reverse_state()) :: reverse_state()
  def exec({:working, [x | xs], ys}) do
    {:working, xs, [x | ys]}
  end

  def exec({:working, [], ys}) do
    {:done, ys}
  end
...
defmodule RevAppend do
  @type rotation_state ::
          {:reversing, [term()], [term()], [term()], [term()]}
          | {:appending, [term()], [term()]}
          | {:done, [term()]}

  @spec start_rotation([term()], [term()]) :: rotation_state()
  def start_rotation(xs, ys) do
    {:reversing, xs, [], ys, []}
  end

  @spec exec(rotation_state()) :: rotation_state()
  def exec({:reversing, [x | xs], ys, [z | zs], as}) do
    {:reversing, xs, [x | ys], zs, [z | as]}
  end

  def exec({:reversing, [], ys, [z], as}) do
    {:appending, ys, [z | as]}
  end
